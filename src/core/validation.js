...
export function validateVersion(version, employees, availabilities) {
  ...
  for (const employee of employees) targetDeviation[employee.id] = (targetDeviation[employee.id] || 0) - (version.daysWorkedTarget?.[employee.id] ?? version.targetDaysWorked ?? 0);
